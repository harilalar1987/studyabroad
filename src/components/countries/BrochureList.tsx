  ...
  if (universities.length === 0) {
    return (
      <div className="text-center py-12 px-4 bg-white/5 rounded-2xl border border-white/10">
        ...
        <h3 className="text-lg font-bold text-white mb-2">Brochures Coming Soon</h3>
    ...
    <div className="space-y-4">
      {universities.map((uni, i) => (
        <motion.div
          ...
          className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl hover:border-sky-400/20 transition-colors"
        >
          <div className="flex-1">
            ...
            {uni.description && (
              <p className="text-gray-400 text-sm mt-1.5 line-clamp-2">{uni.description}</p>
            )}
          </div>
          <div className="flex gap-3">
            {uni.pdfFile ? (
              <a
                ...
                className="px-6 py-2.5 rounded-full bg-sky-400 text-navy text-sm font-medium hover:bg-sky-500 transition-colors whitespace-nowrap"
              >
                View
              </a>
            ) : (
              <span className="px-5 py-2.5 rounded-full bg-white/10 text-gray-500 text-sm font-medium whitespace-nowrap">
                Coming Soon
              </span>
            )}
          </div>
